...
Rules.loadMulti = function(arr, strict)
{
    ...
    if (arr instanceof Array)
    {
        for (var a=0; a<arr.length; a++)
        {
            ...
            if (item.FROM && item.TO !== undefined && item.TO !== null)
            {
...
var process = function(inPath, rules)
{
    ...
    {
        var rls      = rules.all();
        var split    = inPath.split(path.sep);
        ...
        if (split.length > 0)
        {
            ...
            for (var a=0; a<split.length; a++)
            {
                ...
                else if (rls[split[a]] !== undefined && rls[split[a]] !== null)
                {
                    result = result.concat(rls[split[a]]);
                    result = result.concat(path.sep);
                }
                else if (split[a].indexOf('%') === 0 && split[a].length === 2)
                {
                    var num = parseInt(split[a].slice(1), 10);
